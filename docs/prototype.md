...
| **4 Beweisen (Plattform)** | `golden.test.ts` | **405 Golden Vectors, 13 335 Commands** über alle Regelversionen — darunter ein *handgeschriebener* Vektor, der den Kernkreislauf in genau der Reihenfolge durchläuft, um die es im Spiel geht. Der Korpus, den der Mobile-Port an Tag eins abspielt. |
...
| — | `levels.test.ts` | Erfahrung und Stufen: abgeleitet statt gespeichert, nie rückwärts, Schwellen dürfen über Versionen nur sinken. Dazu die Rezept-Staffelung: dass Geld kein Rezept freikauft, dass Späteres besser zahlt, und dass kein Gebäude kaufbar ist, das noch nichts kann. |
